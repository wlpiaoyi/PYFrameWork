...
-(void) setRsController:(PYFwRightSlipController *)rsController{
    ...
    kAssign(self);
    [_rsController setBlockLayoutAnimate:^(PYFrameworkShow frameworkShow, PYFwlayoutParams * _Nullable rootParams, PYFwlayoutParams * _Nullable menuParams) {
        ...
        kStrong(self);
        if(frameworkShow == PYFrameworkShowUnkownShow){
            
        }else if(frameworkShow & PYFrameworkRootFillShow){
            CATransform3D transTemp = CATransform3DScale(trans, 2, 2, 0);
            (*menuParams).transform = transTemp;
            (*menuParams).alpha = .5;
            (*rootParams).transform = trans;
        }else if(frameworkShow & PYFrameworkRootFitShow){
